...
0
|| checktplrefresh('./template/default/forum/forumdisplay_fastpost.htm', './template/default/common/seditor.htm', 1662988106, '1', './data/template/1_1_forum_forumdisplay_fastpost.tpl.php', './template/default', 'forum/forumdisplay_fastpost')
|| checktplrefresh('./template/default/forum/forumdisplay_fastpost.htm', './template/default/forum/seccheck_post.htm', 1662988106, '1', './data/template/1_1_forum_forumdisplay_fastpost.tpl.php', './template/default', 'forum/forumdisplay_fastpost')
|| checktplrefresh('./template/default/forum/forumdisplay_fastpost.htm', './template/default/common/upload.htm', 1662988106, '1', './data/template/1_1_forum_forumdisplay_fastpost.tpl.php', './template/default', 'forum/forumdisplay_fastpost')
|| checktplrefresh('./template/default/forum/forumdisplay_fastpost.htm', './template/default/common/seccheck.htm', 1662988106, '1', './data/template/1_1_forum_forumdisplay_fastpost.tpl.php', './template/default', 'forum/forumdisplay_fastpost')
;?>
...
<?php } ?>
<input type="text" id="subject" name="subject" class="px" value="" onkeyup="dstrLenCalc(this, 'checklen', <?php echo $_G['setting']['maxsubjectsize'];?>);" tabindex="11" style="width: 25em" />
...
<a href="javascript:;" title="ข้อความตัวหนา" class="fbld"<?php if(empty($seditor[2])) { ?> onclick="seditor_insertunit('<?php echo $seditor[0];?>', '[b]', '[/b]');doane(event);"<?php } ?>>B</a>
<?php } if(in_array('color', $seditor[1])) { ?>
<a href="javascript:;" title="ตั้งค่าสีข้อความ" class="fclr" id="<?php echo $seditor[0];?>forecolor"<?php if(empty($seditor[2])) { ?> onclick="showColorBox(this.id, 2, '<?php echo $seditor[0];?>');doane(event);"<?php } ?>>Color</a>
<?php } if(in_array('img', $seditor[1])) { ?>
<a id="<?php echo $seditor[0];?>img" href="javascript:;" title="รูปภาพ" class="fmg"<?php if(empty($seditor[2])) { ?> onclick="seditor_menu('<?php echo $seditor[0];?>', 'img');doane(event);"<?php } ?>>Image</a>
<?php } if(in_array('link', $seditor[1])) { ?>
<a id="<?php echo $seditor[0];?>url" href="javascript:;" title="เพิ่มลิงค์" class="flnk"<?php if(empty($seditor[2])) { ?> onclick="seditor_menu('<?php echo $seditor[0];?>', 'url');doane(event);"<?php } ?>>Link</a>
<?php } if(in_array('quote', $seditor[1])) { ?>
<a id="<?php echo $seditor[0];?>quote" href="javascript:;" title="อ้าง" class="fqt"<?php if(empty($seditor[2])) { ?> onclick="seditor_menu('<?php echo $seditor[0];?>', 'quote');doane(event);"<?php } ?>>Quote</a>
<?php } if(in_array('code', $seditor[1])) { ?>
<a id="<?php echo $seditor[0];?>code" href="javascript:;" title="รหัส" class="fcd"<?php if(empty($seditor[2])) { ?> onclick="seditor_menu('<?php echo $seditor[0];?>', 'code');doane(event);"<?php } ?>>Code</a>
...
<a id="<?php echo $seditor[0];?>at" href="javascript:;" title="@เพื่อน" class="fat"<?php if(empty($seditor[2])) { ?> onclick="seditor_menu('<?php echo $seditor[0];?>', 'at');doane(event);"<?php } ?>>@เพื่อน</a>
...
您需要登录后才可以发帖 <a href="member.php?mod=logging&amp;action=login" onclick="showWindow('login', this.href)" class="xi2">เข้าระบบ</a> | <a href="member.php?mod=<?php echo $_G['setting']['regname'];?>" class="xi2"><?php echo $_G['setting']['reglinkname'];?></a>
<?php } else { ?>
