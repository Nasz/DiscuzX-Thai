...
0
|| checktplrefresh('./template/default/member/login.htm', './template/default/common/seccheck.htm', 1662984553, '1', './data/template/1_1_member_login.tpl.php', './template/default', 'member/login')
...
<a href="member.php?mod=<?php echo $_G['setting']['regname'];?>" class="xi2">ไม่มีบัญชี?<a href="member.php?mod=<?php echo $_G['setting']['regname'];?>"><?php echo $_G['setting']['reglinkname'];?></a></a>
</span>
<?php if(!$secchecklogin2) { ?>
<h3 class="xs2">เข้าระบบ</h3>
<?php } else { ?>
<h3 class="xs2">โปรดป้อนรหัสการยืนยันและทำการบันทึกต่อไป</h3>
<?php } ?>
</div>
<div>
<?php } ?>
...
<?php if(!empty($_GET['infloat'])) { if(!empty($_GET['guestmessage'])) { ?>คุณต้องเข้าสู่ระบบก่อนเพื่อดำเนินการดำเนินการนี้ต่อไป<?php } elseif($auth) { ?>โปรดเพิ่มข้อมูลการเข้าสู่ระบบด้านล่าง<?php } else { ?>เข้าสู่ระบบผู้ใช้<?php } } ?>
</em>
<span><?php if(!empty($_GET['infloat']) && !isset($_GET['frommessage'])) { ?><a href="javascript:;" class="flbc" onclick="hideWindow('<?php echo $_GET['handlekey'];?>', 0, 1);" title="การปิด">การปิด</a><?php } ?></span>
</h3>
...
<tr>
<th>
<?php if($this->setting['autoidselect']) { ?><label for="username_<?php echo $loginhash;?>">บัญชีผู้ใช้:</label><?php } else { ?>
...
<option value="secmobile">หมายเลขโทรศัพท์มือถือรักษาความปลอดภัย</option>
<?php } ?>
</select>
</span>
<?php } ?>
...
<table>
<tr>
<th><label for="password3_<?php echo $loginhash;?>">รหัสผ่าน:</label></th>
...
<td class="tipcol"><a href="javascript:;" onclick="display('layer_login_<?php echo $loginhash;?>');display('layer_lostpw_<?php echo $loginhash;?>');" title="ลืมรหัสผ่าน">ลืมรหัสผ่าน</a></td>
</tr>
</table>
</div>
...
<option value="0"><?php if($questionexist && !$seccodecheck) { ?>คำถามเพื่อความปลอดภัย<?php } else { ?>คำถามเพื่อความปลอดภัย(หากไม่ได้ตั้งค่าโปรดละเว้น)<?php } ?></option>
<option value="1">แม่ชื่อ</option>
<option value="2">ชื่อของคุณปู่</option>
<option value="3">เมืองเกิดของพ่อ</option>
<option value="4">ชื่อครูคนหนึ่งของคุณ</option>
<option value="5">รุ่นคอมพิวเตอร์ส่วนบุคคลของคุณ</option>
<option value="6">ชื่อร้านอาหารที่คุณชื่นชอบ</option>
<option value="7">ตัวเลขสี่หลักสุดท้ายของใบขับขี่</option>
</select></td>
</tr>
</table>
</div>
...
<tr>
<th></th>
<td><label for="cookietime_<?php echo $loginhash;?>"><input type="checkbox" class="pc" name="cookietime" id="cookietime_<?php echo $loginhash;?>" tabindex="1" value="2592000" <?php echo $cookietimecheck;?> />เข้าสู่ระบบอัตโนมัติ</label></td>
</tr>
</table>
</div>
...
<th>&nbsp;</th>
<td>
<button class="pn pnc" type="submit" name="loginsubmit" value="true" tabindex="1"><strong>เข้าระบบ</strong></button>
</td>
<td>
<?php if($this->setting['sitemessage']['login'] && empty($_GET['infloat'])) { ?><a href="javascript:;" id="custominfo_login_<?php echo $loginhash;?>" class="y">&nbsp;<i class="fico-info fic4 fc-p" alt="ช่วย" class="vm"></i></a><?php } ?>								
</td>
</tr>
</table>
</div>
...
<h3 class="flb">
<em id="returnmessage3_<?php echo $loginhash;?>">ลืมรหัสผ่าน</em>
<span><?php if(!empty($_GET['infloat']) && !isset($_GET['frommessage'])) { ?><a href="javascript:;" class="flbc" onclick="hideWindow('login')" title="การปิด">การปิด</a><?php } ?></span>
</h3>
...
<table>
<tr>
<th><label for="lostpw_username">ชื่อผู้ใช้:</label></th>
...
<tr>
<th></th>
<td><button class="pn pnc" type="submit" name="lostpwsubmit" value="true" tabindex="100"><span>ส่ง</span></button></td>
</tr>
</table>
</div>
</div>
...
<em>เข้าสู่ระบบผู้ใช้</em>
<span><a href="javascript:;" class="flbc" onclick="hideWindow('login')" title="การปิด">การปิด</a></span>
<?php } ?>
</h3>
